...
right = function(){
  ...
  # ?milie du Ch?telet - French natural philosopher, mathematician, physicist, and author during the early 1730s, known for her translation of and commentary on Isaac Newton's book Principia containing basic laws of physics. https:#en.wikipedia.org/wiki/%C3%89milie_du_Ch%C3%A2telet
  "chatelet",
  ...
  # Ir?ne Joliot-Curie - French scientist who was awarded the Nobel Prize for Chemistry in 1935. Daughter of Marie and Pierre Curie. https:#en.wikipedia.org/wiki/Ir%C3%A8ne_Joliot-Curie
  "joliot",

  # Karen Sp?rck Jones came up with the concept of inverse document frequency, which is used in most search engines today. https:#en.wikipedia.org/wiki/Karen_Sp%C3%A4rck_Jones
  "jones",
  ...
  # Auguste and Louis Lumi?re - the first filmmakers in history - https:#en.wikipedia.org/wiki/Auguste_and_Louis_Lumi%C3%A8re
  "lumiere",
  ...
  # Ralph C. Merkle - American computer scientist, known for devising Merkle's puzzles - one of the very first schemes for public-key cryptography. Also, inventor of Merkle trees and co-inventor of the Merkle-Damg?rd construction for building collision-resistant cryptographic hash functions and the Merkle-Hellman knapsack cryptosystem. https:#en.wikipedia.org/wiki/Ralph_Merkle
  "merkle",
  ...
  # Tat???yana Avenirovna Proskuriakova (Russian: ???????????????? ?????????????????????? ??????????????????????????) (January 23 [O.S. January 10] 1909 ??? August 30, 1985) was a Russian-American Mayanist scholar and archaeologist who contributed significantly to the deciphering of Maya hieroglyphs, the writing system of the pre-Columbian Maya civilization of Mesoamerica. https:#en.wikipedia.org/wiki/Tatiana_Proskouriakoff
  "proskuriakova",
  ...
  # Wilhelm Conrad R?ntgen - German physicist who was awarded the first Nobel Prize in Physics in 1901 for the discovery of X-rays (R?ntgen rays). https:#en.wikipedia.org/wiki/Wilhelm_R%C3%B6ntgen
  "roentgen",
  ...
  # Lina Solomonovna Stern (or Shtern; Russian: ???????? ?????????????????????? ??????????; 26 August 1878 ??? 7 March 1968) was a Soviet biochemist, physiologist and humanist whose medical discoveries saved thousands of lives at the fronts of World War II. She is best known for her pioneering work on blood???brain barrier, which she described as hemato-encephalic barrier in 1921. https:#en.wikipedia.org/wiki/Lina_Stern
  "shtern",

  # Fran?oise Barr?-Sinoussi - French virologist and Nobel Prize Laureate in Physiology or Medicine; her work was fundamental in identifying HIV as the cause of AIDS. https:#en.wikipedia.org/wiki/Fran%C3%A7oise_Barr%C3%A9-Sinoussi
  "sinoussi",
  ...
  # Christiane N?sslein-Volhard - German biologist, won Nobel Prize in Physiology or Medicine in 1995 for research on the genetic control of embryonic development. https:#en.wikipedia.org/wiki/Christiane_N%C3%BCsslein-Volhard
  "volhard",

  # C?dric Villani - French mathematician, won Fields Medal, Fermat Prize and Poincar? Price for his work in differential geometry and statistical mechanics. https:#en.wikipedia.org/wiki/C%C3%A9dric_Villani
  "villani",
...
# name generation -----
# this is a direct translation of https://github.com/moby/moby/blob/master/pkg/namesgenerator/names-generator.go
#' #' @export
...
getRandomNames = function(n=1,alliterate = FALSE){
  ...
  names = seq_along(nameSets) %>% lapply(function(i){
    ...
    paste0(allPossible[sample(nrow(allPossible),setN,replace = FALSE),] %>% apply(1,paste,collapse = '_'),suffix)
  }) %>% unlist
